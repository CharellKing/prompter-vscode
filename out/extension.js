...
const cellExecutor_1 = require("./cellExecutor");
function activate(context) {
    console.log('Prompter extension is now active!');
    // 注册语言配置
    context.subscriptions.push(vscode.languages.setLanguageConfiguration('prompt', {
        comments: {
            lineComment: '//'
        },
        brackets: [['[', ']'], ['(', ')'], ['{', '}']],
    ...
    // 注册notebook provider
    const notebookProvider = new notebookProvider_1.PrompterNotebookProvider();
    context.subscriptions.push(vscode.workspace.registerNotebookSerializer('prompter-notebook', notebookProvider));
    // 注册notebook controller来处理prompt cell的语言显示
    const controller = vscode.notebooks.createNotebookController('prompter-controller', 'prompter-notebook', 'Prompter Controller');
    ...
    // 监听notebook选择变化
    context.subscriptions.push(controller.onDidChangeSelectedNotebooks((e) => {
        console.log('Notebook selection changed:', e);
    }));
    // 创建cell执行器
    const cellExecutor = new cellExecutor_1.CellExecutor(context);
    // 注册命令
    const createNotebookCommand = vscode.commands.registerCommand('prompter.createNotebook', async () => {
        ...
        // 创建符合新格式的notebook数据
        const cells = [
            new vscode.NotebookCellData(vscode.NotebookCellKind.Markup, '# Welcome to Prompter!\n\nThis is a new Prompter notebook. You can create code and markdown cells to build interactive documents.', 'markdown'),
            new vscode.NotebookCellData(vscode.NotebookCellKind.Code, '// Welcome to Prompter!\n// Press Ctrl+Enter to run this cell\nconsole.log("Hello, World!");', 'javascript')
        ];
        // 为每个cell添加ID
    ...
    const createPromptCellCommand = vscode.commands.registerCommand('prompter.createPromptCell', async () => {
        ...
        if (editor) {
            ...
            // 创建新的提示词cell
            const newCell = new vscode.NotebookCellData(vscode.NotebookCellKind.Markup, '请在此输入您的提示词内容...', 'prompt');
            newCell.metadata = {
    ...
    context.subscriptions.push(createNotebookCommand, runCellCommand, runAllCellsCommand, createPromptCellCommand);
    // 注册状态栏项
